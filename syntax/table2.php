...
class syntax_plugin_approve_table2 extends DokuWiki_Syntax_Plugin {
    ...
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('----+ *approve table 2 *-+\n.*?----+', $mode,'plugin_approve_table2');
    }
    ...
    public function renderXhtml(Doku_Renderer $renderer, $params)
    {
		...
		//New: Added revision.Version of Page
		...
		*/
        ...
        // Output Table
		//New table design
		$class = 'plugin__approve_table';
		$renderer->doc .= '<table class="'.$class.'"><tr>';
		...
		*/
		...
		$renderer->doc .= '<th>' . $this->getLang('hdr_rev') . '</th>';
        $renderer->doc .= '<th class="text-center">' . $this->getLang('hdr_updated') . '</th>';
		/*Original Approver class in table. 
		...
		*/
        ...
        foreach($pages as $page) {
			...
			$last_approved_rev = $helper->find_last_approved($sqlite, $page['page']);
			$last_approved_ver = $helper->find_last_approved_ver($sqlite, $page['page']);
			$last_approved = strtotime($helper->find_last_approved_app($sqlite, $page['page']));
            $id = $page['page'];
            ...
            /*Original Script to include Namespaces as headings
            $pageNS = getNS($id);
			...
			*/
			...
			if ($last_approved_rev != '') {
				if ($currentState == 'odd') {
					$class = 'plugin__approve_neutral_odd';
				} else {
					$class = 'plugin__approve_neutral_even';
				}
			...
			//New IF for adjusting the Hyperlink
			if ($last_approved_rev != '') {
				$renderer->doc .= '&rev='.$last_approved_rev.'">';
			} else {
				$renderer->doc .= '">';
			}
			...
			/*Original Script to implement Last modified User
			...
			*/
			...
			//New Column for Revision
			$renderer->doc .= ''.$last_approved_ver.'';
			$renderer->doc .= '</td><td align="center">';
			//New IF for adjusting the Hyperlink
			if ($last_approved_rev != '') {
				$renderer->doc .= '<a href="' . wl($id) . '&rev='.$last_approved_rev.'">' . strftime('%m / %Y',$date) . '</a>';
			} else {
				$renderer->doc .= '<a href="' . wl($id) . '">' . strftime('%m / %Y',$date) . '</a>';
			}
			//New Script
            $renderer->doc .= '</td></tr>';
